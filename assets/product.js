  ...
  class VariantSelects extends HTMLElement {
    ...
    updateOptions() {
      ...
      this.fieldsets.forEach((fieldset, i) => {
        if (fieldset.querySelector('select')) {
          ...
          console.log(fieldset.querySelector('input:checked').dataset.option)
          if (fieldset.querySelector('input:checked').dataset.option) {
            this.selectedVariantGroup = fieldset.querySelector('input:checked').dataset.option;
          }
        }
      });
    ...
    updateVariantText() {
      ...
      fieldsets.forEach((item, i) => {
        const allLabels = item.querySelectorAll('.form__label__value');
    
        allLabels.forEach(label => {
          const labelOption = label.getAttribute('data-option');
    
          if (item.dataset.splits === "true") {
            if (labelOption === this.selectedVariantGroup) {
              label.innerHTML = this.options[i];
            } else if (labelOption) {
              // Clear labels with a data-option that doesn't match
              label.innerHTML = '';
            }
            // Labels without data-option are untouched
          } else if (!labelOption) {
            // If splits is not true and no data-option, update as usual
            label.innerHTML = this.options[i];
          }
        });
      });
    ...
    setDisabled() {
      ...
      if (variant_data) {
        ...
        this.fieldsets.forEach((fieldset, i) => {
          ...
          console.log("fieldset_options", fieldset_options)
          ...
          if (fieldset_options) {
            ...
            if (fieldset.querySelector('select')) {
              ...
              // This code has been replaced since the metafield allows you to move the variants around, the input array didn't match the variant array
              //fieldset.querySelectorAll('input').sort().forEach((input, input_i) => {
                //console.log("Option input: ", input, "Input index: ", input_i, fieldset_options[input_i])
                //input.classList.toggle('is-disabled', fieldset_options[input_i].isUnavailable);
              //});
              Array.from(fieldset.querySelectorAll('input'))
              .sort((a, b) => a.value.localeCompare(b.value))
              .forEach(input => {
                const matchingOption = fieldset_options.find(option => option.value === input.value);
                input.classList.toggle('is-disabled', matchingOption?.isUnavailable || false);
              });
            }
          }
        });
